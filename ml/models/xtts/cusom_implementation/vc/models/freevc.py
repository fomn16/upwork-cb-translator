...
class FreeVC(BaseVC):
    ...
    def new_load_audio(self, path) -> Tensor:
        audio, sr = librosa.load(path, mono=True)
        audio = librosa.resample(audio, orig_sr=sr, target_sr=self.config.audio.input_sample_rate)
        audio = torch.from_numpy(audio)
        return audio.float().to(self.device)
    
    @torch.inference_mode()
    def get_speaker_embedding(self, src):
        wav_tgt = self.new_load_audio(src).cpu().numpy()
        g_tgt = self.enc_spk_ex.embed_utterance(wav_tgt)
        return torch.from_numpy(g_tgt)[None, :, None].to(self.device)
    
    @torch.inference_mode()
    def voice_conversion_preprocessed(self, src:Tensor, speaker_embedding:Tensor):
        """
        Voice conversion pass of the model.

        Args:
            src (torch.Tensor): Source utterance. Words to be used in output
            tgt (torch.Tensor): Target mel encoding, voice to be used in output.

        Returns: Output numpy array.
        """
        # src
        c = self.extract_wavlm_features(src[None, :])
        audio = self.inference(c, g=speaker_embedding)
        audio = audio[0][0].data.cpu().float().numpy()
        return audio
