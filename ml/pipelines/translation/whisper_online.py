# code from https://github.com/ufal/whisper_streaming/blob/main/whisper_online.py
...
class OnlineASRProcessor:
    ...
    def insert_audio_chunk(self, audio_bytes: bytes):
        """
        audio_bytes: raw PCM16-LE bytes, interleaved if num_channels>1.
        Converts to float32 in [-1,1], downmixes to mono if needed,
        and appends to self.audio_buffer.
        """
        samples_int16 = np.frombuffer(audio_bytes, dtype='<i2')
        samples_float32 = samples_int16.astype(np.float32) / 32768.0
        self.audio_buffer = np.concatenate([self.audio_buffer, samples_float32])
