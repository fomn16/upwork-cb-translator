...
class MLXWhisper(ASRBase):
    ...
    def set_translate_task(self):
        self.transcribe_kargs["task"] = "transcribe"
...
class VACOnlineASRProcessor(OnlineASRProcessor):
    ...
    def process_iter(self):
        if self.is_currently_final:
            ...
            # Suppress the VAD status output unless in debug mode
...
def asr_factory(args, logfile=sys.stderr):
    ...
    if backend == "openai-api":
        ...
        asr = OpenaiApiASR(lan=args.language)
    else:
...
def output_transcript(o, now=None):
    """Process Whisper output and return/print the transcribed text.
    
    Args:
        o: Whisper output tuple (start_time, end_time, text)
        now: Optional timestamp
    
    Returns:
        str: The transcribed text (or None if no text)
    """
    if o[0] is not None:  # If there's valid text
        return o[2]  # Return the text
    return None  # No text case