...
def translate(audio: bytes, sample_rate: int):
    ...
    translated_wav, text = translator1.translate_chunk(
        audio, input_sample_rate=sample_rate, sample_width=sample_width, channels=channels
    )
    end_time = time.time()
...
def pump_audio(ff_in: Popen, ff_out: Popen, output_queue: OutputAudioQueue, segment_size: int, sample_rate: int, sdp_path: str):
    buf = b""
    try:
        while True:
...
async def initiate_translation(data: TranslationRequest):
    ...
    ff_out = run_ffmpeg_output(
        "127.0.0.1", data.outputPort, data.payloadType, data.ssrc
    )
